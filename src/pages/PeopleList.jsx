...
export default function PeopleList() {
  ...
  const driverList = isDriver && currentUser.driver_profile ? passengerList : tripsList;
  ...
  const waitingList = isDriver ? useRequests(currentUser.driver_profile?.id) : [];
  function renderList() {
    const renderedWaitingList =
      isDriver && waitingList && waitingList.length > 0
    ...
    const renderedMainList = mainList?.map((item) => {
      if (isDriver && item.booked_trips) {
