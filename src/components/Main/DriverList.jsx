...
export default function DriverList({ list, toggleCreating }) {
  ...
  const waitingList = isDriver && driverId ? useRequests(driverId) || [] : [];

  function renderLength() {
    if (activeDrives) {
      return isDriver ? activeDrives.length : filteredDrives.length;
    }
    return 0;
  }
  ...
  function renderList() {
    ...
    if (list.length > 0) {
      ...
      return <span> {isDriver ? "У вас нет пассажиров" : "Сейчас нет активных водителей"}</span>;
    }
    ...
    <Footer className={`bg-[#F6F6F6] flex justify-center`}>
      <div className='w-full h-full flex flex-col justify-between'>
        ...
        <div className='flex flex-col  gap-5 pb-6'>
          <div
            className='flex max-w-[350px] justify-end items-end text-right text-[14px] leading-[16.1px] '
